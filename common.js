...
var fileServerUrl = "http://file1.hncaixinwang.cn";
...
function Upload(action, repath, uppath, iswater, isthumbnail,form) {
    ...
    if (action == "ExcelFile")
    {
        sendUrl = "/Core/upload_ajax.ashx?action=" + action + "&ReFilePath=" + repath + "&UpFilePath=" + uppath;
    }
    else
    {
        sendUrl = fileServerUrl + "/Core/upload_ajax.ashx?action=" + action + "&ReFilePath=" + repath + "&UpFilePath=" + uppath;
    }

    //判断是否打水印
    if (arguments.length == 4) {
        sendUrl = fileServerUrl + "/Core/upload_ajax.ashx?action=" + action + "&ReFilePath=" + repath + "&UpFilePath=" + uppath + "&IsWater=" + iswater;
    }
    //判断是否生成宿略图
    if (arguments.length == 5 || arguments.length == 6) {
        ...
        {
            sendUrl = fileServerUrl + "/Core/upload_ajax.ashx?action=" + action + "&ReFilePath=" + repath + "&UpFilePath=" + uppath + "&IsWater=" + iswater + "&IsThumbnail=" + isthumbnail;
        }
    }
...
function AttachUpload(repath, uppath) {
    var submitUrl = fileServerUrl + "/Core/upload_ajax.ashx?action=AttachFile&UpFilePath=" + uppath;
    //开始提交
    $("form").ajaxSubmit({
        beforeSubmit: function (formData, jqForm, options) {
            //隐藏上传按钮
...
function AttachUpdate(repath, uppath) {
    ...
    var submitUrl = fileServerUrl + "/Core/upload_ajax.ashx?action=AttachFile&UpFilePath=" + btnNewName;
    //开始提交
    $("form").ajaxSubmit({
        beforeSubmit: function (formData, jqForm, options) {
            //隐藏上传按钮
