...
function addImage(bigSrc, smallSrc) {
    var hideFocusPhoto = $('#hideFocusPhoto').val();
    var newLi = $('<li></li>');
    var claname = '';
    if (hideFocusPhoto == '') {
        $('#hideFocusPhoto').val(smallSrc);
        claname = 'selected';
    }
    ...
    var newImg = $('<div onclick="setFocusImg(this);" class=\"img-box ' + claname + '">' +
...
function del_img(obj) {
    var focusphoto = $("#hideFocusPhoto");
    var smallimg = $(obj).prevAll(".img-box").children("img").eq(0).attr("src");
    var node = $(obj).parent(); //要删除的LI节点
    node.remove(); //删除DOM元素
    //检查是否为封面
    if (focusphoto.val() == smallimg) {
        focusphoto.val("");
        var firtimg = $("#show_list ul li .img-box img").eq(0);
        firtimg.parent().addClass("selected"); //取第一张做为封面
...
function setFocusImg(obj) {
    $("#hideFocusPhoto").val($(obj).children("img").eq(0).attr("src"));

    $("#show_list ul li .img-box").removeClass("selected");
    ...
    $.messager.prompt('图片描述', '批量设置请在最前面输入：@', function (r) {
        if (r) {
            if (r.indexOf("@") > -1) {
                ...
                $(".img-box").find(".remark i").html(r);
            } else {
                $(hidRemark).val(r);
                $(obj).prevAll(".img-box").find(".remark i").html(r);
            }
        }
    });
